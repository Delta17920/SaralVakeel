      ...
      <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            { label: 'High Risk', sub: 'Action needed', value: reports.filter(r => parseFloat(r.riskScore.toString()) >= 8).length, icon: AlertTriangle, color: isDarkMode ? 'text-[#C0392B]' : 'text-[#E74C3C]', bg: isDarkMode ? 'bg-[#C0392B]/10' : 'bg-[#E74C3C]/10', border: isDarkMode ? 'bg-[#1E1C1A] border-[#2B2E35]' : 'bg-white border-[#C8BEB4] shadow-md' },
            { label: 'Completed', sub: 'Ready', value: reports.filter(r => r.status === 'complete').length, icon: CheckCircle, color: isDarkMode ? 'text-[#27AE60]' : 'text-[#2ECC71]', bg: isDarkMode ? 'bg-[#27AE60]/10' : 'bg-[#2ECC71]/10', border: isDarkMode ? 'bg-[#1E1C1A] border-[#2B2E35]' : 'bg-white border-[#C8BEB4] shadow-md' },
            { label: 'Avg Risk', sub: 'Overall', value: reports.length > 0 ? (reports.reduce((a, r) => a + parseFloat(r.riskScore.toString()), 0) / reports.length).toFixed(1) : '0.0', icon: TrendingUp, color: 'text-[#9B59B6]', bg: 'bg-[#9B59B6]/10', border: isDarkMode ? 'bg-[#1E1C1A] border-[#2B2E35]' : 'bg-white border-[#C8BEB4] shadow-md' },
          ].map((s, i) => (
